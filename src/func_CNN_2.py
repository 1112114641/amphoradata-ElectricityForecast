...
def func_CNN_2(df_, nn_name='func_CNN', single_output=False):
    ...
    all1 = LeakyReLU(alpha=0.001)(all1)

    def cnn_lrelu_block(name_in, filters=64, kernel=5, padding='causal', lrelualpha=0.01, name=''):
        '''
    ...
    drop_NSW1 = Dropout(0.15)(NSW1)
    QLD1 = cnn_lrelu_block(all1, filters=128, kernel=3, name='QLD1')
    drop_QLD1 = Dropout(0.15)(QLD1)
    VIC1 = cnn_lrelu_block(all1, filters=128, kernel=3, name='VIC1')
    drop_VIC1 = Dropout(0.15)(VIC1)
    SA1 = cnn_lrelu_block(all1, filters=128, kernel=3, name='SA1')
    drop_SA1 = Dropout(0.15)(SA1)
    ...
    def dense_block(name_in,start,mid,end,dropout=0.15, name=''):
        Name2 = Dense(start)(name_in)
        name21 = LeakyReLU(alpha=0.01)(Name2)
        drop_3 = Dropout(dropout)(name21)
        Name4 = Dense(mid)(drop_3)
        name41 = LeakyReLU(alpha=0.01)(Name4)
        output_dense = Dense(end, name=name)(name41)
        return output_dense
    
    NSW3 = dense_block(NSW2,256,64,1, name='NSW')
    QLD3 = dense_block(QLD2,256,64,1, name='QLD')
    VIC3 = dense_block(VIC2,256,64,1, name='VIC')
    SA3 = dense_block(SA2,256,64,1, name='SA')
